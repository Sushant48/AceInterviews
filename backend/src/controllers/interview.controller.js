...
const submitAnswer = asyncHandler(async (req, res) => {
  ...
  if (!interviewId || !questionId || !userAnswer) {
    throw new ApiError(
      400,
      "Interview ID, question ID, and user answer are required"
    );
  }
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, question, "Answer submitted successfully"));
});
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        feedback,
        "Interview completed and feedback generated successfully"
      )
    );
});
...
const getPerformanceMetrics = asyncHandler(async (req, res) => {
  ...
  const strengths = interviews.flatMap(
    (interview) => interview.interviewFeedback.strengths
  );
  const weaknesses = interviews.flatMap(
    (interview) => interview.interviewFeedback.weaknesses
  );
  ...
  const performanceTrend = interviews
    ...
    .filter(
      (interview) =>
        interview.interviewFeedback.overallScore !== null &&
        interview.interviewFeedback.overallScore !== undefined
    )
    .map((interview) => ({
  ...
  const recentInterviews = interviews.filter(
    (interview) => interview.createdAt >= thirtyDaysAgo
  ).length;

  const recentScores = interviews
    .filter((interview) => interview.createdAt >= thirtyDaysAgo)
    .map((interview) => interview.interviewFeedback.overallScore)
    .filter((score) => score !== null && score !== undefined);

  const previousScores = interviews
    .filter((interview) => interview.createdAt < thirtyDaysAgo)
    .map((interview) => interview.interviewFeedback.overallScore)
    .filter((score) => score !== null && score !== undefined);

  const recentAvgScore = recentScores.length
    ? recentScores.reduce((acc, score) => acc + score, 0) / recentScores.length
    : 0;

  const previousAvgScore = previousScores.length
    ? previousScores.reduce((acc, score) => acc + score, 0) /
  ...
  const bestInterview = interviews.find(
    (interview) => interview.interviewFeedback.overallScore === bestScore
  );

  const bestScoreDate = bestInterview ? bestInterview.createdAt : null;
  ...
  res
    ...
    .json(
      new ApiResponse(
        ...
        "Real-time interview session initialized"
      )
    );
});
